...
.\scripts\test-attendance-automation-alert-incident-detail.ps1 -ActivityLogId 123 -Secret "your-secret"
"@
}

$uri = (
    $BaseUrl.TrimEnd("/") +
    "/api/automation/attendance/alerts/incidents/" +
    $ActivityLogId
)
...
Write-Host ""
Write-Host `
    "Attendance automation alert snapshot detail" `
    -ForegroundColor Cyan

Write-Host "Endpoint: $uri"
Write-Host "Activity log ID: $ActivityLogId"
...
switch ($statusCode) {
    200 {
        Write-Host (
            "Automation alert snapshot detail was retrieved."
    ...
    default {
        Write-Host (
            "The snapshot detail request failed."
        ) -ForegroundColor Red
