...
.\scripts\test-attendance-automation-alert-incident-sla.ps1 -Secret "your-secret"
"@
}

$uri = (
    $BaseUrl.TrimEnd("/") +
    "/api/automation/attendance/alerts/incidents/sla"
)
...
Write-Host ""
Write-Host `
    "Attendance automation alert incident SLA" `
...
switch ($statusCode) {
    200 {
        Write-Host (
            "Automation incident SLA is not breached."
    ...
    503 {
        Write-Host (
            "One or more automation incident SLA targets are breached."
    ...
    default {
        Write-Host (
            "The incident SLA request failed."
        ) -ForegroundColor Red
