...
.\scripts\test-attendance-automation-lock-diagnostics.ps1 -Secret "your-secret"
"@
}

$uri = (
    $BaseUrl.TrimEnd("/") +
    "/api/automation/attendance/lock-diagnostics"
)
...
Write-Host ""
Write-Host (
    "Attendance automation lock diagnostics"
...
switch ($statusCode) {
    200 {
        Write-Host (
            "MySQL named-lock diagnostics completed."
    ...
    503 {
        Write-Host (
            "The lock diagnostic completed, but one or more checks failed."
    ...
    default {
        Write-Host (
            "The lock diagnostic request failed."
        ) -ForegroundColor Red
