...
export function buildAttendanceAuditCsv(result: AttendanceAuditResult): string {
  const summaryRows = [
    ...
    ["Manual Created", result.summary.manualCreated],
    ["Manual Corrected", result.summary.manualCorrected],
    ["Missing Timeout", result.summary.missingTimeout],
    ["Verified", result.summary.verified],
    ["Approved", result.summary.approved],
    ["Status Updated", result.summary.statusUpdated],
    ["Current Page Logs", result.pagination.totalItems],
